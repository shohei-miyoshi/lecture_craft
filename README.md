...
現在の開発環境では `backend/data/lecture_craft_app.db` の SQLite を使い、
...
ローカルデバッグ用のフォールバックとして、`~/.config/lecture_craft/openai_api_key`、
`~/.config/lecture_craft/openai_api_key.txt`、`~/.config/lecture_craft/apikey.txt` を優先して参照します。
互換のため、従来の `~/.config/kenkyu/...` も引き続き読めます。
