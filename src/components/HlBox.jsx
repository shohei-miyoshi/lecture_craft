...
export default function HlBox({ hl, isSel, isPlay, wrapRef, dispatch }) {
  ...
  return (
    <div onMouseDown={onMoveStart} style={{
      ...
      border: `2px solid ${c}`, background: bg, borderRadius: 3, cursor: "grab", zIndex: isSel ? 10 : 5,
      boxShadow: isSel ? `0 0 0 2px ${c}55` : "none",
      animation: isPlay ? "lc-pulse 1s ease-in-out infinite alternate" : "none",
    }}>
      {/* ラベル */}
      <div style={{ position: "absolute", top: -16, left: 0, background: c, color: "#fff", fontFamily: "var(--fm)", fontSize: 8, padding: "1px 4px", borderRadius: 2, pointerEvents: "none", whiteSpace: "nowrap" }}>
        {KIND_LABEL[hl.kind]}
      </div>
      {/* 削除ボタン（選択時のみ） */}
      {isSel && (
        <div onClick={(e) => { e.stopPropagation(); dispatch({ type: "RM_HL_ID", v: hl.id }); }}
          style={{ position: "absolute", top: -16, right: 0, width: 16, height: 16, background: "var(--rdd)", border: "1px solid var(--rd)", borderRadius: 3, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, color: "var(--rd)" }}>
          ×
        </div>
      )}
      {/* 8方向リサイズハンドル（選択時のみ） */}
