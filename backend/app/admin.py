...
def _snapshot_row_limit() -> int:
    try:
        return max(100, int(os.getenv("LECTURE_CRAFT_ADMIN_SNAPSHOT_LIMIT", "5000")))
    except Exception:
        return 5000
...
def build_admin_overview(limit: int = 12) -> Dict[str, Any]:
    ...
    return {
        ...
        "limits": {
            "snapshot_row_limit": _snapshot_row_limit(),
        },
        "research": {
...
def _load_job_snapshots() -> List[Dict[str, Any]]:
    return _sort_snapshot_rows(_load_job_snapshots_from_db(), "updated_at")


def _load_export_events() -> List[Dict[str, Any]]:
    return _sort_snapshot_rows(_load_export_events_from_db(), "created_at")


def _load_research_sessions() -> List[Dict[str, Any]]:
    return _sort_snapshot_rows(_load_research_sessions_from_db(), "saved_at")


def _load_job_snapshots_from_db() -> List[Dict[str, Any]]:
    try:
        with db_conn() as conn:
            rows = conn.execute(
                """
                SELECT id, kind, status, progress, message, request_json,
                       error_json, partial_result_json, cancel_requested,
                       project_id, generation_run_id, created_at, updated_at
                FROM jobs
                ORDER BY updated_at DESC, id DESC
                LIMIT :limit
                """,
                {"limit": _snapshot_row_limit()},
            ).fetchall()
    except Exception:
        return []
        ...
        jobs.append(
            {
                ...
                "message": row["message"],
                "payload": _loads(row["request_json"]) or {},
                "error": _loads(row["error_json"]),
                "partial_result": _loads(row["partial_result_json"]),
...
def _load_export_events_from_db() -> List[Dict[str, Any]]:
    try:
        with db_conn() as conn:
            rows = conn.execute(
                """
    ...
    for row in rows:
        ...
        payload = {
            **payload,
            ...
            "created_at": row["created_at"],
            "status": row["status"],
            "export_type": "research_jsonl",
            "snapshot_path": f"db://research_export_snapshots/{row['id']}",
        }
        events.append(payload)
    return events


def _load_research_sessions_from_db() -> List[Dict[str, Any]]:
    try:
        with db_conn() as conn:
            rows = conn.execute(
                """
                SELECT id, payload_json
                FROM research_session_snapshots
    ...
    for row in rows:
        ...
        if not isinstance(payload, dict):
            continue
        payload["snapshot_path"] = f"db://research_session_snapshots/{row['id']}"
        sessions.append(payload)
    return sessions


def _sort_snapshot_rows(rows: List[Dict[str, Any]], field: str) -> List[Dict[str, Any]]:
    return sorted(rows, key=lambda row: _parse_dt(row.get(field)), reverse=True)
