...
""".strip()
    ...
    research_block = ""
    if research_context_text.strip():
        research_block = f"""
[Research Context]
以下は LectureCraft の実験条件，KG利用条件，過去の修正傾向です。
スライド内容と矛盾しない範囲で，説明順序，補足量，言い換え方の参考にしてください。
--- 研究コンテキストここから ---
{research_context_text.strip()}
--- 研究コンテキストここまで ---
""".strip()
...
def generate_lecture_scripts(
    ...
    research_context_path = text_output_dir / "research_generation_context.txt"
    if research_context_path.exists():
        with research_context_path.open("r", encoding="utf-8") as f:
            research_context_text = f.read().strip()
        print(f"[info] research_generation_context を読み込みました: {research_context_path}")
    else:
