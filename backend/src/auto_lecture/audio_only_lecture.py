...
def step_stitch_script(
    ...
    if research_context_text.strip():
        prompt_text += (
            ...
            "以下は実験条件と過去の修正傾向です。音声講義として自然であることを優先し，"
            "該当する場合だけ説明順序，補足量，言い換え方の参考にしてください。\n"
            f"{research_context_text.strip()}\n"
        )
...
def generate_audio_only_lecture(
    ...
    research_context_path = Path(paths.output_dir) / "research_generation_context.txt"
    research_context_text = research_context_path.read_text(encoding="utf-8") if research_context_path.exists() else ""
