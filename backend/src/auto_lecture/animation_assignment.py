...
from .tts_simple import clean_script_text
...
def split_sentences(text: str) -> List[str]:
    cleaned = clean_script_text(text or "")
    return [s.strip() for s in _SENT_SPLIT_RE.split(cleaned) if s and s.strip()]
...
def build_user_message(
    ...
    script_full: str,
    sentences: List[str],
    region_overview_path: Optional[str | Path] = None,
) -> Dict[str, Any]:
    """
    ...
    visual_inputs = [
        "- スライド全体の元画像を添付しています。",
        "- 領域の位置と番号は、領域JSONの region_id / coordinates を基準に判断してください。",
    ]
    if region_overview_path:
        visual_inputs.append("- 現在の全領域を赤枠と番号で示した編集後の領域一覧画像を添付しています。")
    if region_imgs:
        visual_inputs.append("- 各 region_id の内容を確認できる領域参照画像を添付しています。")
    else:
        visual_inputs.append("- 個別の領域参照画像はありません。元画像と領域JSONを使って判断してください。")
...
[備考]
- JSON，編集後の領域一覧画像，領域参照画像で同じ region_id を使用しています。
...
"""
    ...
    content: List[Dict[str, Any]] = [
        ...
        {"type": "input_text", "text": "[slide_image] スライド全体の元画像"},
        {"type": "input_image", "image_url": slide_img_url},
    ]

    if region_overview_path:
        try:
            overview_url = image_to_dataurl(region_overview_path)
        except Exception:
            overview_url = None
        if overview_url:
            content.append(
                {
                    "type": "input_text",
                    "text": "[edited_region_overview] ユーザ編集後の全領域を赤枠と番号で示した画像",
                }
            )
            content.append({"type": "input_image", "image_url": overview_url})
...
def normalize_mapping_payload(
    ...
    *,
    ...
    """
    LLM が返した対応付け結果を、runner が扱える旧形式に寄せる。
    """
    ...
    for i, s in enumerate(sents):
        ...
        if animate is not None and isinstance(animate, dict):
            ...
            if animate is not None and animate.get("style") not in STYLE_CATALOG:
                animate = None
...
def generate_mapping_for_slide(
    client: OpenAI,
    *,
    ...
    script_full: str,
    sentences: List[str],
    region_imgs: Optional[List[Dict[str, str]]] = None,
    region_overview_path: Optional[str | Path] = None,
) -> Dict[str, Any]:
    user_msg = build_user_message(
        ...
        region_imgs=region_imgs or [],
        script_full=script_full,
        sentences=sentences,
        region_overview_path=region_overview_path,
    )
    ...
    return normalize_mapping_payload(mapping, slide_str=slide_str, sentences=sentences)
...
def run_animation_assignment(client: OpenAI, paths: ProjectPaths, explanations: List[str]) -> None:
    ...
    for sidx0, slide_path in enumerate(slide_imgs):
        ...
        try:
            mapping = generate_mapping_for_slide(
                client,
