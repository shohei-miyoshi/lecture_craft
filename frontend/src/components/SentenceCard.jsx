...
export default function SentenceCard({
  ...
  const playSentence = () => {
    sel();
    if (!playOnSelect || !onPlaySentence) return;
    if (window.getSelection?.()?.toString()) return;
    onPlaySentence(sent.id);
  };
      ...
      <div style={{ padding: "9px 12px", paddingLeft: showHl ? 36 : 12 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 6, flexWrap: "wrap" }}>
          ...
          <span
            ...
            title={playOnSelect ? "クリックしてこの文から再生" : undefined}
            style={{ fontFamily: "var(--fm)", fontSize: 9, color: "var(--tm)", background: "var(--s3)", padding: "1px 5px", borderRadius: 3, cursor: playOnSelect ? "pointer" : "default" }}
          >
            {playOnSelect ? "▶ " : ""}{fmt(sent.start_sec)}–{fmt(sent.end_sec)}
          </span>
          {!showHl && (
            <button
        ...
        {timingOpen && !showHl && <TimingEditor sent={sent} dispatch={dispatch} addToast={addToast} />}
...
function TimingEditor({ sent, dispatch, addToast }) {
  const [start, setStart] = useState(String(sent.start_sec));
  const [end,   setEnd]   = useState(String(sent.end_sec));

  useEffect(() => {
    setStart(String(sent.start_sec));
    setEnd(String(sent.end_sec));
  }, [sent.end_sec, sent.start_sec]);

  const apply = () => {
    const s = parseFloat(start);
    const e = parseFloat(end);
    if (isNaN(s) || isNaN(e) || s < 0 || e < 0 || e <= s) {
      addToast?.("er", "タイミングは 0 秒以上で、終了は開始より後にしてください");
      return;
    }
    dispatch({ type: "PUSH_HISTORY" });
