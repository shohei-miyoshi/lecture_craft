...
export default function HlEditor({
  ...
  const curKind = hl?.kind ?? (drawSentId === sid ? drawKind : null);
  ...
  const setKind = (kind) => {
    ...
    dispatch({ type: "SET", k: "drawKind", v: kind });
  ...
  const kindBtnStyle = (kind) => {
    ...
    return {
      ...
      fontFamily: "var(--fb)",
      fontSize: 11,
      cursor: linkOnly ? "not-allowed" : "pointer",
      textAlign: "center",
      opacity: linkOnly ? 0.55 : 1,
    };
  };
      ...
      <div style={{ background: "var(--s2)", padding: 10 }}>
        {hl ? (
          ...
          <div style={{ marginBottom: 8, fontSize: 10, color: "var(--tm)" }}>
            {linkOnly
              ? "この文はまだどの領域にも対応していません。領域確認済みの枠から対応先を選んでください。"
