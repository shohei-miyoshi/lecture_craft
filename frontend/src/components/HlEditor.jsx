      ...
      <div style={{ background: "var(--s2)", padding: 10 }}>
        {hl ? (
          <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
            <MiniSlide hl={hl} slide={slide} dispatch={dispatch} />
            <div style={{ flex: 1 }}>
          ...
          <div>
            <div style={{ fontSize: 10, color: "var(--tm)", marginBottom: 8 }}>
              この文にはまだハイライト枠が対応していません。既存の枠を選ぶか、新規に作成してください。
            </div>
...
function HighlightLinkPanel({ sentence, current, availableHighlights, onLink }) {
  return (
    <div style={{ borderTop: "1px solid var(--bd)", paddingTop: 8 }}>
      ...
      {current && (
        <div style={{ padding: "6px 8px", borderRadius: 6, background: "rgba(110,193,255,.10)", border: "1px solid rgba(110,193,255,.18)", fontSize: 10, color: "var(--tp)", marginBottom: 6 }}>
          現在の枠: #{current.id} / {(current.sentence_ids ?? []).length} 文に対応
        </div>
      )}
      {availableHighlights.length === 0 ? (
        <div style={{ fontSize: 10, color: "var(--tm)" }}>使える既存枠はありません</div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          {availableHighlights.map((item) => (
            <button
              ...
              style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, padding: "6px 8px", borderRadius: 6, border: "1px solid var(--bd)", background: "var(--s3)", color: "var(--tp)", fontSize: 10 }}
            >
              <span>#{item.id} / X:{Math.round(item.x)} Y:{Math.round(item.y)} W:{Math.round(item.w)} H:{Math.round(item.h)}</span>
