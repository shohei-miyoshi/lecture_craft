...
export default function SlideCanvas({
  ...
  const actSent = correspondenceEnabled
    ? (
        state.previewActiveSentenceId
  ...
  const onWheel = (e) => {
    ...
    if (nextSlide !== state.curSl) {
      dispatch({ type: "SEEK_SLIDE", v: nextSlide });
    }
  };
  ...
  const onStageMouseDown = (e) => {
    if (state.drawMode && !highlightEditable) return;
    if (!state.drawMode) {
      startPan(e);
      return;
    }
  ...
  const onStageDoubleClick = (e) => {
    if (state.drawMode || !showBB || !highlightEditable) return;
    const selectedSentence = correspondenceEnabled
      ? state.sents.find((s) => s.id === state.selSent)
      : null;
    const point = getImagePercentPoint(e.clientX, e.clientY);
    if (!point) return;
    const region = {
  ...
  useEffect(() => {
    if (!state.drawMode || !highlightEditable) return undefined;
    const onMove = (e) => {
    ...
    const onUp = (e) => {
      ...
      if (state.drawSentId) {
        const kind = state.drawKind ?? findHighlightForSentence(state.hls, state.drawSentId)?.kind ?? "marker";
