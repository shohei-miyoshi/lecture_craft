...
import { isPdfFile } from "../utils/helpers.js";
...
export default function ProjectHome({
  ...
  onOpenProject,
  onResumeEditing,
  onStoredProjectRenamed,
  onStoredProjectDeleted,
  currentProject,
  requestConfirm,
  requestPrompt,
  addToast,
  ...
  const currentProjectId = currentProject?.id ?? currentData?.project_meta?.id ?? null;
  const isGeneratingWorkspace = currentStatus === "proc";
  const currentWorkspaceLabel = currentProjectId ? "現在の作業" : "未保存の一時作業";
  const isCurrentProject = (projectId) => Boolean(projectId && currentProjectId && projectId === currentProjectId);

  const handlePendingPdf = (file) => {
    if (isGeneratingWorkspace) {
      addToast?.("er", "生成中は新しいPDFを選択できません。先に生成を停止してください");
      return;
    }
    if (!isPdfFile(file)) {
  ...
  const handleDelete = async (projectId) => {
    if (isGeneratingWorkspace && isCurrentProject(projectId)) {
      addToast?.("er", "生成中の作業は削除できません。先に生成を停止してください");
      return;
    }
    let project = null;
    try {
      ...
      console.warn("Failed to load project before delete:", error);
      addToast?.("er", error.message || "削除対象のプロジェクトを確認できませんでした");
      return;
    }
    if (!project) {
    ...
    const closesCurrentWorkspace = isCurrentProject(projectId);
    const currentDirtyWarning = closesCurrentWorkspace && currentData?.is_dirty
      ? "\n現在開いている未保存の編集内容も閉じます。"
      : closesCurrentWorkspace
        ? "\n現在開いている作業も閉じます。"
        : "";
    requestConfirm({
      title: "プロジェクトを削除",
      message: `「${project.name}」を保存済みプロジェクトから削除しますか？\nサーバ保存がある場合は、そちらもあわせて削除されます。${currentDirtyWarning}`,
      confirmLabel: "削除",
      onConfirm: async () => {
        try {
          ...
          addToast?.("er", error.message || "プロジェクト削除に失敗しました");
        }
      },
    });
  };
    ...
    requestPrompt?.({
      ...
      onConfirm: async (value) => {
        ...
        try {
          ...
          onStoredProjectRenamed?.(project.id, updated?.name ?? nextName);
          setRefreshKey((v) => v + 1);
          addToast?.("ok", `プロジェクト名を「${nextName}」に変更しました`);
        } catch (error) {
          addToast?.("er", error.message || "プロジェクト名の変更に失敗しました");
        }
      },
    });
  };
  ...
  const startProjectWithPdf = () => {
    if (isGeneratingWorkspace) {
      addToast?.("er", "生成中は新しいプロジェクトを開始できません。先に生成を停止してください");
      return;
    }
    ...
    >
      <div style={{ maxWidth: 1240, margin: "0 auto", padding: "26px 28px 48px" }}>
        ...
        >
          ...
          <div style={{ display: "grid", gridTemplateColumns: currentData ? "repeat(auto-fit, minmax(280px, 1fr))" : "1fr", gap: 22, position: "relative", zIndex: 1 }}>
            <div>
              ...
              <div style={{ marginTop: 22, maxWidth: 720 }}>
                <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr)", gap: 12, alignItems: "stretch" }}>
                  <div
                    onDragOver={(e) => {
                      e.preventDefault();
                      if (!isGeneratingWorkspace) setDraggingPdf(true);
                    }}
                  ...
                  style={{
                    ...
                    border: `1px dashed ${isGeneratingWorkspace ? "rgba(232,169,75,.34)" : pendingPdf ? "rgba(76,175,130,.58)" : draggingPdf ? "rgba(110,193,255,.52)" : "rgba(110,193,255,.26)"}`,
                    background: isGeneratingWorkspace
                      ? "linear-gradient(135deg, rgba(232,169,75,.10), rgba(255,255,255,.03))"
                      : pendingPdf
                    ...
                    cursor: isGeneratingWorkspace ? "not-allowed" : "pointer",
                    overflow: "hidden",
                ...
                >
                  ...
                  <div style={{ fontFamily: "var(--ff)", fontSize: 20, lineHeight: 1.15, marginBottom: 8 }}>
                    {isGeneratingWorkspace ? "生成中は新規作成できません" : pendingPdf ? "選択した PDF で新規作成" : "PDF を選んで新規作成"}
                  </div>
                  <div style={{ fontSize: 11, color: "var(--ts)", lineHeight: 1.7, marginBottom: 12 }}>
                    {isGeneratingWorkspace
                      ? "現在の生成を停止または完了してから、新しいプロジェクトを開始できます。"
                      : pendingPdf
                ...
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center", justifyContent: "flex-start", marginTop: 12 }}>
                  <button
                    ...
                    disabled={!pendingPdf || isGeneratingWorkspace}
                    style={{
                      ...homeButtonStyle("primary", !pendingPdf || isGeneratingWorkspace),
                      boxShadow: pendingPdf && !isGeneratingWorkspace
                  ...
                  <button
                    ...
                    disabled={isGeneratingWorkspace}
                    style={homeButtonStyle("secondary", isGeneratingWorkspace)}
                  >
                    空のプロジェクトを作成
                  </button>
                </div>
              ...
              >
                ...
                <div style={{ fontSize: 11, color: "var(--ts)", lineHeight: 1.7, marginBottom: 12 }}>
                  {isGeneratingWorkspace
                    ...
                    : currentProjectId
                      ? "保存済みプロジェクトの編集中状態を保持しています。続きからすぐ再開できます。"
                      : "まだ保存されていない作業を保持しています。不要な場合は編集画面でリセットできます。"}
                </div>
          ...
          <section style={{ position: "relative", paddingTop: 10 }}>
            <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 12, marginBottom: 14 }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: "var(--tm)" }}>保存済みプロジェクトの一覧です</div>
              </div>
            ...
            {projects.length === 0 ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {projects.map((project, index) => {
                  const projectIsCurrent = isCurrentProject(project.id);
                  const projectIsGeneratingCurrent = projectIsCurrent && isGeneratingWorkspace;
                  const data = {
                    mode: project.mode,
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 16, alignItems: "center" }}>
                        <div>
                          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6, flexWrap: "wrap" }}>
                            ...
                            {projectIsCurrent ? (
                              <span style={{ padding: "3px 8px", background: "rgba(76,175,130,.1)", color: "var(--gr)", fontSize: 10, fontFamily: "var(--fm)" }}>
                                現在の作業
                              </span>
                            ) : null}
                            <button
                        ...
                        <div style={{ display: "flex", gap: 8, justifySelf: "end", flexWrap: "wrap" }}>
                          ...
                          >
                            {projectIsCurrent ? "戻る" : "開く"}
                          </button>
                          <button
                            onClick={() => handleDelete(project.id)}
                            disabled={projectIsGeneratingCurrent}
                            title={projectIsGeneratingCurrent ? "生成中の作業は削除できません" : undefined}
                            style={{ ...homeButtonStyle("danger", projectIsGeneratingCurrent), padding: "9px 12px", fontSize: 11 }}
                          >
                            削除
                          </button>
                        </div>
