...
export default function HlBox({ hl, isSel, isActive, isPlaying, editable = true, frame, dispatch, requestConfirm, slideHighlights = [] }) {
  ...
  // ── 移動 ──
  const onMoveStart = (e) => {
    if (!editable || isPlaying || e.target.dataset.rh) return;
    e.stopPropagation();
  ...
  const onResizeStart = (e, dir) => {
    ...
    let moved = false;
    let historyPushed = false;
    let last = { x: orig.x, y: orig.y, w: orig.w, hv: orig.h };
    const mv = (ev) => {
      ...
      if (!moved && x === orig.x && y === orig.y && w === orig.w && h === orig.h) return;
      if (!historyPushed) {
        dispatch({ type: "PUSH_HISTORY" });
        historyPushed = true;
      }
      moved = true;
      last = { x, y, w, hv: h };
      dispatch({ type: "UPD_HL", id: hl.id, x, y, w, hv: h, transient: true });
    };
    const up = () => {
      if (moved) {
        dispatch({ type: "UPD_HL", id: hl.id, ...last });
  ...
  return (
    <div data-hl-interactive="1" onMouseDown={onMoveStart} style={{
      ...
      background: bg,
      borderRadius: 3,
      cursor: !editable || isPlaying ? "default" : "grab",
      ...
      {editable && isSel && !isPlaying && Object.entries(HANDLE_POS).map(([dir, pos]) => (
