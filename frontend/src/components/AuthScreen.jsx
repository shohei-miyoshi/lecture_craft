...
export default function AuthScreen({ onAuthenticated, addToast }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const message =
        tab === "login"
          ? (isAdmin ? "管理者としてログインしました" : "ログインしました")
          : (isAdmin ? "管理者アカウントを作成しました" : "アカウントを作成しました");
      ...
      const message = error.message === "Failed to fetch"
        ? "認証サーバーに接続できませんでした。バックエンドの起動状態と VITE_API_URL を確認してください。"
        : (error.message || "認証に失敗しました");
      setFeedback({ kind: "er", message });
      addToast?.("er", message);
    } finally {
      setBusy(false);
    }
    ...
    >
      <div style={panelStyle()}>
        ...
        <div style={{ fontSize: 12, color: "var(--ts)", lineHeight: 1.7, marginBottom: 18 }}>
          {isRegister
            ? "ユーザ名とパスワードでアカウントを作成し、保存済みプロジェクトや研究ログをユーザ単位で管理します。"
          ...
          {feedback ? (
            <div
              style={{
                marginTop: 2,
                padding: "10px 12px",
                border: `1px solid ${feedback.kind === "er" ? "rgba(224,91,91,.45)" : "rgba(76,175,130,.42)"}`,
                background: feedback.kind === "er" ? "rgba(42,24,24,.9)" : "rgba(24,42,34,.9)",
                color: feedback.kind === "er" ? "#ffb3b3" : "#98e0b8",
                fontSize: 11,
                lineHeight: 1.6,
              }}
            >
        ...
        <div style={{ marginTop: 14, fontSize: 10, color: "var(--tm)", lineHeight: 1.7 }}>
          管理者アカウントはサーバ起動時のブートストラップ設定でのみ作成されます。通常の新規登録は一般ユーザとして扱われます。
        </div>
      </div>
    </div>
  );
