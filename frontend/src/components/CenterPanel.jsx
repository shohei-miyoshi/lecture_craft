...
export default function CenterPanel({
  ...
  usePlayback(state, dispatch, { disabled: previewPlayback.audioActive });
  ...
  const correspondenceEnabled = isHl && (reviewStage === "assignment" || reviewStage === "editor");
  const showHighlights = isHl && (
    reviewStage === "layout"
    || reviewStage === "layout_waiting_script"
    || reviewStage === "assignment_generating"
    || reviewStage === "assignment"
    || (reviewStage === "editor" && state.prevMode === "hl")
  );
  const highlightEditable = isHl && !editorLockedByGeneration && (
    reviewStage === "layout" || reviewStage === "editor"
  );
    ...
    <main style={{ flex: 1, display: "flex", flexDirection: "column", background: "transparent", minWidth: 0, overflow: "hidden", position: "relative" }}>
      ...
      {isAudio ? (
        <AudioView
          ...
          sentenceTimings={previewPlayback.sentenceTimings}
        />
      ) : (
        <div
          style={{
      ...
      {/* ── 再生バー ── */}
      {previewPlayback.finalRender.videoUrl && (
        <div style={{ padding: "8px 12px", borderTop: "1px solid rgba(255,255,255,.05)", background: "rgba(76,175,130,.08)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
          <div style={{ fontSize: 10, color: "var(--ts)" }}>
            本番確認プレビューを生成済みです。必要に応じて動画を開いて最終確認できます。
          </div>
          <button
            onClick={previewPlayback.openFinalRenderPreview}
            style={{ padding: "5px 10px", borderRadius: 999, border: "1px solid rgba(76,175,130,.35)", background: "rgba(76,175,130,.14)", color: "var(--tp)", fontSize: 10 }}
          >
            本番動画を開く
          </button>
        </div>
      ...
      <Playbar
        ...
        highlightTicksEnabled={correspondenceEnabled}
        previewPlayback={previewPlayback}
        finalRenderEnabled={reviewStage === "editor" && !isAudio}
        playbackDisabled={state.status === "proc"}
      />
