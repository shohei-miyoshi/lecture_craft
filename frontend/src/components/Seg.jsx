 ...
 *   onChange: (value) => void
 */
export default function Seg({ opts, val, onChange, disabled = false }) {
  return (
    <div style={{ display: "flex", background: "var(--s2)", border: "1px solid var(--bd)", borderRadius: "var(--r)", overflow: "hidden", opacity: disabled ? 0.62 : 1 }}>
      {opts.map((o, i) => (
        <button
          key={o.v}
          disabled={disabled}
