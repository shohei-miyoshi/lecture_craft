...
export function buildResearchSnapshot(state, trigger, extra = {}) {
  ...
  const project = {
    id: extra.project?.id ?? state.projectMeta?.id ?? null,
    name: extra.project?.name ?? state.projectMeta?.name ?? null,
    version_number: extra.project?.version_number ?? state.projectMeta?.version_number ?? null,
    updated_at: extra.project?.updated_at ?? state.projectMeta?.updated_at ?? null,
  };
  const extensions = {
  ...
  return {
    version: "2026-07-04_research_v2",
