...
export function usePlayback(state, dispatch, options = {}) {
  ...
  useEffect(() => {
    if (disabled || !playing || totDur <= 0) return;

    // 再生開始 or シーク後の基準を記録
    let lastWall = Date.now();
    let lastT    = refs.current.curT;
    let id;
  ...
  }, [disabled, playing, playSpeed, totDur, state.seekSignal]);  // seekSignal はシーク時にインクリメントするカウンタ
}
